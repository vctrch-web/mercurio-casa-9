...
async function loadExisting() {
  ...
  if (existingPost) {
    ...
    dateInput.value = existingPost.date ? existingPost.date.slice(0, 10) : new Date().toISOString().slice(0, 10);
...
function collectPostData() {
  ...
  // Hora local com offset — evita que o Supabase interprete como UTC
  const now = new Date();
  ...
  const date = `${dateVal}T${pad(now.getHours())}:${pad(now.getMinutes())}:00${offsetStr}`;
